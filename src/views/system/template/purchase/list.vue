          ...
          <div class="table-tool">
            ...
            <div class="table-tool-btn" v-if="currentRouterData">
              <div class="btn-con" v-for="(item, index) in currentRouterData.menuBtn" >
                <el-button v-if="item.menuCode =='templatePurchaseCreate' " class="tool-basic-btn" size="mini" @click="handleAdd">{{item.menuName}}</el-button>
                <el-button v-else-if="item.menuCode =='templatePurchaseCheckOrEdit'"  size="mini" class="tool-edit-btn" @click="handleEdit">{{item.menuName}}</el-button>
                <el-button v-else-if="item.menuCode =='templatePurchaseDelete'" size="mini" class="tool-delete-btn" @click="handleDelete">{{item.menuName}}</el-button>
                <el-button v-else-if="item.menuCode =='templatePurchaseExport'" class="tool-export-btn" size="mini" @click="handleDownload">{{item.menuName}}</el-button>
              </div>
            </div>
          </div>
...
<script>
  import {FetchResultTemplate} from '@/api/sys.template.purchase'//api
  ...
  export default {
    ...
    mounted () {
      this.$nextTick(() => { // 关闭当前右侧的 tab 页
        this.closeRight({pageSelect: '/template/purchase'})
      })
    },
    computed: {
    ...
    methods: {
      ...
      fetchList () {
        ...
        FetchResultTemplate('get',Object.assign({
          ...
          let respondData = res.data
           this.resultTemplateDtos = respondData.resultTemplateDtos;
             this.pageInfo = {
               ...this.pageInfo,
      ...
      handleEdit (index, row) {
        if(this.multipleSelection.length === 1){
          this.$router.push({ name: 'template-purchase-edit' , params: { resultTemplateId: this.multipleSelection[0].resultTemplateId }})
        }else{
          this.$message({
            type: 'info',
      ...
      handleAdd () {
        this.$router.push({ path: '/template/purchase-add' })
      },
      /**
       * 下载图片
       * */
