...
export default {
  ...
  actions: {
    ...
    login ({ commit, dispatch }, {
      ...
      console.log('1111', username, password, code)
        ...
        AccountLogin({
          ...
          .then(async res => {
            ...
            if (res.message === 'success') {
              ...
              util.cookies.set('token', respondData.token)
              let permissionsData = respondData.roles[0].permissions
              let treeData = util.composeMenuTree(permissionsData)
              await dispatch('d2admin/menu/menuDataSet', treeData, { root: true })
              // 设置 vuex 用户信息
              await dispatch('d2admin/user/set', respondData, { root: true })
